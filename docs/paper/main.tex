...
\[
  ...
  \;+\;\bigoh\!\left(\bigl(1-\tfrac1m\bigr)^{\!n}\right).
\]
...
-H_{2m}^{(2)}$ (limit $\tfrac32-\pi^2/6$) and the geometric decay rate $1-1/m$, by a direct
...
\begin{equation}\label{eq:main}
  \Fg(n,m)\;=\;\frac{H_{2m}}{2m}\,n\;+\;b(m)\;+\;\bigoh\!\left(\bigl(1-\tfrac1m\bigr)^{n}\right),
  \qquad
...
(\S\ref{sec:slope}), the geometric \emph{fade rate} $1-1/m$ (\S\ref{sec:fade}), and the exact
...
The remainder in Theorem~\ref{thm:main} is $\bigoh((1-\tfrac1m)^n)$; equivalently, the
dominant subdominant eigenvalue of the guessing dynamics is exactly $1-1/m$.
\end{proposition}
...
skip $\ell\to\ell+2$, the source of the factor $2$ in the exponent. Thus the correction is
$\Theta((1-\tfrac1m)^n)$. At $m=1$ this rate is $0$: the expansion is exact for $n\ge2$.
\end{proof}
...
with probability $1-\bigoh(1)$ per position, and is exact in the limit), the first-occurrence
...
$C$-finite with characteristic roots $\{i/m\}\cup\{(2i-1)/2m\}$ (spectral gap $1/m$),
confirming the fade rate $1-1/m$ by a second route. (iv) The block-model bin decomposition
